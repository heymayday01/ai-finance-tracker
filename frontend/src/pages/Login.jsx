  ...
  useEffect(() => {
    ...
    for (let i = 0; i < STAR_COUNT; i++) {
      stars.push({
        ...
        hue: Math.random() > 0.7 ? (Math.random() * 60 + 180) : 0, // some colored, some white
    ...
    function animate() {
      ...
      const nebulaGrad = ctx.createRadialGradient(mx, my, 0, mx, my, 350)
      nebulaGrad.addColorStop(0, 'rgba(0, 212, 255, 0.06)')
      nebulaGrad.addColorStop(0.4, 'rgba(120, 80, 255, 0.03)')
      ...
      for (const star of stars) {
        // Mouse interaction: push stars away gently
        const dx = star.x - mx
        const dy = star.y - my
        ...
        const drawX = star.x + star.ox
        const drawY = star.y + star.oy
        ...
        if (star.baseSize > 1.2) {
          const glowGrad = ctx.createRadialGradient(drawX, drawY, 0, drawX, drawY, size * 6)
          if (star.hue > 0) {
            glowGrad.addColorStop(0, `hsla(${star.hue}, 80%, 70%, ${alpha * 0.15})`)
          } else {
            glowGrad.addColorStop(0, `rgba(255, 255, 255, ${alpha * 0.1})`)
          }
    ...
    <>
      ...
      <div className="login-root">
        ...
        <div className={`login-card ${mounted ? 'mounted' : ''}`}>
          ...
          <h1 className="login-heading">Welcome<br /><span>back.</span></h1>
          <p className="login-sub">Sign in to your financial dashboard</p>
          ...
          <form onSubmit={handleSubmit} autoComplete="on">
            <div className={`field ${focused === 'email' ? 'focused' : ''}`}>
              ...
              <input className="field-input" id="login-email" name="email" type="email"
            ...
            <div className={`field ${focused === 'password' ? 'focused' : ''}`}>
              ...
              <input className="field-input" id="login-password" name="password" type="password"
                placeholder="••••••••"
